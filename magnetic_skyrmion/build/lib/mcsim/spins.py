...
class Spins:
    ...
    def __init__(self, n, value=(0, 0, 1)):
        ...
        if len(value) != 3:
            raise ValueError(
                f"Length of iterable value must be 3, not {len(n)=}.")
    ...
    def mean(self):
        ...
        mx = np.sum(self.array[:, :, 0])/(self.n[0] *
                                          self.n[1])  # See source 3 in refrences
        # calculate the mean for the y component
        my = np.sum(self.array[:, :, 1])/(self.n[0]*self.n[1])
        # calculate the mean for the z component
        mz = np.sum(self.array[:, :, 2])/(self.n[0]*self.n[1])
    ...
    def __abs__(self):
        ...
        norm = np.ones((self.array.shape[0], self.array.shape[1], 1))
        for i in range(self.array.shape[0]):
            for j in range(self.array.shape[1]):
                norm[i, j, 0] = np.linalg.norm(
                    self.array[i, j])  # assign the norm value, also see source 9
        return norm
    ...
    def plot(self):
        x, y = np.meshgrid(
            np.arange(self.array.shape[0]), np.arange(self.array.shape[1]))  # see source 4
